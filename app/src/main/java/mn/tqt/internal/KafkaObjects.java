...
public class KafkaObjects {
    ...
    KafkaObjects(QuerySchema schema) {
        this.schema = schema;
        objects = new ArrayList<>();
    }

    void add(ObjectNode objectNode) {
        objects.add(applySchema(objectNode));
    }

    public List<ObjectNode> toList() {
        return objects;
    }


    private ObjectNode applySchema(ObjectNode json) {
            List<JsonPointer> pointers = switch (schema.type()) {
                case EXCLUDE -> constructSchemaPointers(json.deepCopy(), schema);
                case INCLUDE -> {
                    List<JsonPointer> schemaPointers =
                            constructSchemaPointers(json.deepCopy(), schema);
